...
batch_size = 8
input_shape = (224, 224)
base_path = "/home/cy/whale_data/train/"
...
train, test = train_test_split(data, test_size=0.2, shuffle=True, random_state=1337)
...
history = model.fit_generator(gen(train_gen), validation_data=gen(test_gen), epochs=50, verbose=2, workers=1,
                              callbacks=callbacks_list, steps_per_epoch=2500, validation_steps=300)
...
train_files = glob.glob("/home/cy/whale_data/train/*.jpg")
test_files = glob.glob("/home/cy/whale_data/test/*.jpg")
...
train_preds_vec = open("./train_preds_vec.txt","w")
train_preds_vec.write(str(train_preds))
ptint("train_preds:"+train_preds)
...
test_preds_vec = open("./test_preds_vec.txt","w")
test_preds_vec.write(str(test_preds))
ptint("test_preds:"+test_preds)
...
for filepath, distance, neighbour_ in zip(test_file_names, distances_test, neighbors_test):
    ...
    sample_result.append(("new_whale", 0.0))
...
df.to_csv("sub_0.38_%s.csv" % model_name, index=False)
...
for ind in range(len(l_image_name_test)) :
    ...
    vect_dist.append(("new_whale", 0.00))  # add "new_whale" ecach time
...
df.to_csv("sub_0.42_%s.csv" % model_name, index=False)
